...
class UnNormalize(object):
    """
    Un-normalize an image tensor using the given mean and standard deviation.

    Args:
        mean (sequence): Sequence of means for each channel in the input tensor.
        std (sequence): Sequence of standard deviations for each channel in the input tensor.
    """
    ...
    def __call__(self, tensor):
        ...
        for t, m, s in zip(tensor, self.mean, self.std):
            # Multiply each channel of the input tensor by its standard deviation and add its mean.
            # This is the inverse of the normalize operation.
            t.mul_(s).add_(m)
